...
- (id)init
{
	...
	if (parsed == NO)
	{
		...
		[sourceCollection setParsingOption: SCKDetailedPreprocessingRecord];
...
- (void)testStaticFunction
{
	...
	UKIntsEqual(1, (int)[[sourceFile functions] count]);
...
- (void)testStaticVariable
{
	...
	NSDictionary *variables = [sourceFile variables];	

	UKObjectsEqual(A(@"staticVar1", @"staticVar2"), [[[variables allValues] mappedCollection] name]);

	UKIntsEqual(2, (int)[variables count]);

	SCKGlobal *staticVar1 = [variables objectForKey: @"staticVar1"];
	SCKGlobal *staticVar2 = [variables objectForKey: @"staticVar2"];
	SCKFunction *function2 = [[self parsedFunctionsForNames: A(@"function2")] firstObject];
